...
cp -r /etc/authselect /etc/pam.d_bakup_$tiempo
...
password    requisite     pam_pwhistory.so debug use_authtok remember=24 retry=3
...
auth        required      pam_env.so
auth        required      pam_faillock.so preauth silent audit deny=8
auth        sufficient    pam_unix.so try_first_pass nullok
auth        [default=die] pam_faillock.so authfail audit deny=8
auth        required      pam_deny.so
